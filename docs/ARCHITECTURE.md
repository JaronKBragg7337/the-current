...
The guarantee is scoped to simulation engine `0.3.0`, schema `2`, a saved seed/configuration, and the same ordered external inputs. Replay is exact only after the entropy and other inputs that actually occurred are known. The many-future verifier intentionally supplies different not-yet-recorded entropy streams to the same genesis and requires their outcomes to diverge. Unsupported schema versions fail explicitly.

## Daily simulation order and spatial causality

At a high level, the engine applies queued inputs, evaluates migration from settlement attraction and unknowable outside pressure, updates movement and needs, resolves work/economy/social/lifecycle systems, advances construction/institutions/breakthroughs/artifact study, then emits a daily summary and projection. The exact rule order is documented in [SIMULATION_RULES.md](SIMULATION_RULES.md).

People have authoritative positions, previous positions, destinations, homes, and job/project assignments. Movement occurs before effects. Work, trade, research, care, governance, and construction require arrival at the relevant building/site. Encounters require physical proximity or a shared home/worksite. Births occur at the mother/home position. A person can travel at most 140 metres per world day and currently moves directly toward the destination; rendered roads do not yet constrain the path. The shared spectator uses the real 24-hour day clock to expose these authoritative journeys at staggered times instead of teleporting between daily snapshots.
...
Ordinary laborers and explorers choose sanitation when accumulated waste justifies it; a severe public-health load can recruit a broader one-day community response. Workers travel to named high-severity sites and remove waste only after arriving and succeeding. Energy strengthens collection and transport provides durable capacity rather than being destroyed as fuel. New construction inherits nearby soil, water, and contamination instead of creating pristine land. Farms, wells, workshops, and power stations consume declared inputs, and production is curtailed when expected use plus physical storage cannot accept more output. Well production is mixed with stored water to persist an authoritative drinking-water quality whose health effect is scaled by the amount each resident actually receives.
