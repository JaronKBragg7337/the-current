...
A scheduled edge function (`supabase/functions/the-current-tick`) is the only writer of the world: Era One advances at one world day per real day with fresh entropy generated only when that day resolves, guarded by an optimistic day-number lock. The resulting event and snapshot are publicly inspectable after the fact; the redundant raw entropy audit table remains service-role only. All clients — development and production alike — poll and project the snapshot; `?world=local` is an explicit opt-in that runs a private local world instead, for an outsider who wants their own. Time controls do not exist in either mode: every world advances one world day per real day from its own genesis. The engine is imported into the function from the public repository pinned to an exact commit via esm.sh.
